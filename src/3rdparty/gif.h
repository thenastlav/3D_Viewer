...
int GifPartitionByMean(uint8_t* image, int left, int right, int com,
                       int neededMean) {
  if (left < right - 1) {
...
void GifSplitPalette(uint8_t* image, int numPixels, int treeNode, int treeLevel,
  ...
  GifSplitPalette(image, subPixelsA, treeNode * 2, treeLevel + 1,
                  buildForDither, pal);
  GifSplitPalette(image + subPixelsA * 4, subPixelsB, treeNode * 2 + 1,
                  treeLevel + 1, buildForDither, pal);
}
